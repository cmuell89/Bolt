...
from database.database import IntentsDatabase, ExpressionsDatabase
from app import app
...
class ClassificationTest(unittest.TestCase):
    ...
    def test_classify_route(self):
        ...
        second_response = self.app.post('/classification/classify', data=json.dumps(dict(query='What is order 2313?')), headers=second_test_headers)
...
class ExpressionsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app.testing = True
        self.access_token = os.environ.get("ACCESS_TOKEN")
        self.intents_db = IntentsDatabase()
        self.expressions_db = ExpressionsDatabase()
        self.intents_db.add_intent('test-intent')
        self.expressions_db.add_expressions_to_intent('test-intent',
                                          ["test expression one", "test expression two", "test expression three",
                                           "test expression four"])
        self.intents_db.add_intent('delete-intent')
        self.expressions_db.add_expressions_to_intent('delete-intent', ["one", "two", "three", "four"])

    def tearDown(self):
        self.intents_db.delete_intent('delete-intent')
        self.intents_db.delete_intent('test-intent')
        self.intents_db.close_database_connection()
        self.expressions_db.close_database_connection()
...
class UnlabeledExpressionsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.unlabeled_test_expression_ID = [tup[0] for tup in self.db_results if
                                             tup[1] == self.post_unlabeled_test_expression]
...
class ArchivedExpressionsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.archived_test_expression_ID = [tup[0] for tup in self.db_results if
                                            tup[1] == self.post_archived_test_expression]
